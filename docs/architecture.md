...
sequenceDiagram
    ...
    G->>A: Verify JWT
    G->>T: Send Money Request
    T->>W: Debit Sender
    W-->>T: Debit Confirmation
    T->>W: Credit Receiver
    W-->>T: Credit Confirmation
    T->>T: Create Transaction Record
    T->>R: Publish transaction.completed
    R->>N: Event
    N->>Email/SMS: Send Notifications
    T-->>G: Transaction Success
